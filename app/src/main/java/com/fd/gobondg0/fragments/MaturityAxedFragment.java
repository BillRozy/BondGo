...
public class MaturityAxedFragment extends AxedFragment {
    ...
    protected void prepareData(ForecastView graphicalCanvas, Map<String, ForecastResult> results) {
        forecastVar = (float) BaseApp.getCalculator().getMaturity();
        ...
        for(String type : results.keySet()) {
            if (results.get(type) != null) {
                ...
                arrays.add(res.mMaturityCalls);
                arrays.add(res.mMaturityPuts);
            }
        }
        ...
        graphicalCanvas.setAxisLegend("Maturity, year", "Price, $");
    }
    ...
    protected void buildForecast(ForecastView graphicalCanvas, PlotLegend legendary, Map<String, ForecastResult> results) {
        ...
        for(String type : results.keySet()) {
            if (results.get(type) != null) {
                ForecastResult res = results.get(type);
                graphicalCanvas.createPolyline(Ts, res.mMaturityCalls, type + " Call", mColors.get(type)[0]);
                graphicalCanvas.createPolyline(Ts, res.mMaturityPuts, type + " Put", mColors.get(type)[1]);
